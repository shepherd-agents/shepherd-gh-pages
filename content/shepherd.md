...
Run two coding agents in parallel to ship a feature faster, and they'll happily edit the same file blind to each other, then clobber each other's work. The fix is another agent: a supervisor watching both, ready to step in before they collide. That's a **meta-agent**, and the strongest agentic systems increasingly look like this.^[Examples like: Anthropic's Claude Code composes [dynamic workflows](https://code.claude.com/docs/en/workflows) of sub-agents, Nous Research's Hermes delegates to [agent teams](https://hermes-agent.nousresearch.com/docs/user-guide/features/delegation), and Kimi K2.5 coordinates an [agent swarm](https://arxiv.org/abs/2602.02276).]
